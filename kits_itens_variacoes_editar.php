            ...
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        ...
                        $sql = "select eki.id ekiid, eki.item ekiitem, ekv.id ekvid, ekv.item ekvitem, ekv.variacao ekvvariacao  FROM eventos_kit_item as eki inner join eventos_kit_variacoes as ekv on eki.id = ekv.item where ekv.id = '$pegaid'";
                        $result = mysqli_query($conn, $sql);

                        while ($row = mysqli_fetch_assoc($result)) {

                            echo "<h2 class='content-header-title float-left mb-0'>$row[ekiitem]</h2>";
                            echo "<div class='breadcrumb-wrapper col-12'>";
                            echo "<ol class='breadcrumb'>";
                            echo "<li class='breadcrumb-item'><a href='index.php'>Home</a></li>";
                            echo "<li class='breadcrumb-item'><a href='kits_itens_variacoes.php?id=$row[ekiid]'>Itens do kit</a></li>";
                            echo "<li class='breadcrumb-item active'>Editar</li>";
                            echo "</ol>";
                            echo "</div>";
                        }
                        mysqli_close($conn);
                            ...
                            <div class="card-content">
                                <div class="card-body">
                                    <form method="post" enctype="multipart/form-data"
                                          action="functions/kits_itens_variacoes_editar.php">

                                        <?php
                                        require("connections/conn.php");
                                        $pegaid = (int)$_GET['id'];

                                        $sql = "select * FROM eventos_kit_variacoes where id = '$pegaid'";
                                        $result = mysqli_query($conn, $sql);
                                        ...
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            echo "<p>Variação</p>";
                                            echo "<input type='text' class='form-control' name='variacao' id='basicInput' value='$row[variacao]'>";
                                            echo "</fieldset>";
                                            echo "</div>";
                                            echo "</div>";
